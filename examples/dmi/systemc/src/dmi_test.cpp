...
void dmi_test::dmi_test_sequence() {
  ...
  while (true) {
    ...
    bool dmi_ptr_valid = bus_initiator_socket->get_direct_mem_ptr (*payload, dmi_data );
    if (!dmi_ptr_valid) {
    ...
    std::memcpy (&read_data_word, dmi_data.get_dmi_ptr() + (test_address - dmi_data.get_start_address()), 4);
    ...
    // Acquire DMI pointer for test address
    payload->set_address (test_address);
    dmi_ptr_valid = bus_initiator_socket->get_direct_mem_ptr (*payload, dmi_data );
    if (!dmi_ptr_valid) {
      fprintf (stderr, "get_direct_mem_ptr() did not succeed\n");
      assert (false);
      return;
    ...
    std::memcpy (dmi_data.get_dmi_ptr() + (test_address - dmi_data.get_start_address()), &write_data_word, 4);
