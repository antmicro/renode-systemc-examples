# DMI native interface example (`dmi-native`)

On Linux, the MappedMemory peripheral can be configured to use a region of
memory that can be shared with another process, or a library in the Renode
process, via a memory mapped file (MMF).
...
When MappedMemory has the sharedMemory attribute set, the SystemC module
can request a pointer by using get\_direct\_mem\_ptr().  When this method
is called, Renode passes a pointer to the mapping into the module so that
it can directly access the mapped memory.
