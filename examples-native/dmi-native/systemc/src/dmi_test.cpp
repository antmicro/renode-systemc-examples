...
dmi_test::dmi_test(sc_core::sc_module_name name) {
  ...
  // cause heap corruption. See renode_memory_transport::b_transport -> renode_write_bytes_to_bus.
  set_stack_size(8 << 20);
...
void dmi_test::dmi_test_sequence() {
  ...
  while (true) {
    ...
    if (payload->get_response_status() != tlm::TLM_OK_RESPONSE) {
      std::cerr << "Failed to write to system bus\n";
      assert(false);
      return;
    }
    if (!payload->is_dmi_allowed()) {
      std::cerr << "Target did not set DMI hint\n";
      assert(false);
      return;
    }
    ...
    if (!dmi_ptr_valid) {
      std::cerr << "get_direct_mem_ptr() did not succeed\n";
      assert(false);
      return;
    }
    ...
    if (read_data_word == write_data_word) {
      std::cout << "DMI read data matched\n";
    } else {
      std::cerr << "DMI read data mismatched\n";
      assert(false);
      return;
    }

    std::cout << "Second test: write via DMI, read via b_transport\n";
    ...
    renode_invalidate_translation_blocks(test_address, test_address + 4);
    ...
    bus_initiator_socket->b_transport(*payload, delay);
    if (payload->get_response_status() != tlm::TLM_OK_RESPONSE) {
      std::cerr << "Failed to read from system bus\n";
      assert(false);
      return;
    }
